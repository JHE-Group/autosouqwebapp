...
5. Hatla2ee's `/price-limit/{n}` ladder concept (§3.1). Then do what they didn't: make the counts correct, add schema, cross-link the tiers, and extend to city and model. **Two later corrections to this item.** Their register — "affordable", "budget-friendly", never "cheap" — is no longer ours to copy: the ban on "cheap"/رخيصة was lifted 28 Jul 2026 (see NICHE.md), because autocomplete shows رخيصة is what Omani buyers actually type. And their ladder is *labelled* "under {n}" but returns adjacent **ranges** underneath — measured: `/price-limit/2500` shares 18 of 20 listings with `/price-limit/2000`, while `/price-limit/3000` shares 0. Copy the range mechanics, not the "under X" label, which on this site would mean advertising a ceiling while hiding the cheapest cars under it.
