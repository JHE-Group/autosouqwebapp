 ...
 *
 * Was a pair of anchors, one of them `href="#"` — so choosing the grid pushed a
 * `#` onto the URL and jumped the page to the top, and neither told an assistive
 * technology which view was currently on. They are buttons in a radio-style
 * group now, with `aria-pressed` carrying the state the `.active` class was
 * carrying visually.
 */
...
export default function ListGridToggler({ isGrid, setIsGrid }) {
  ...
  return (
    <div className="asq-viewswitch" role="group" aria-label={t("view")}>
      <button
        type="button"
        className={`btn-view grid asq-viewswitch__btn ${isGrid ? "active" : ""}`}
        aria-pressed={isGrid}
        onClick={() => setIsGrid(true)}
      >
        <GridIcon />
        <span className="asq-sr-only">{t("gridView")}</span>
      </button>
      <button
        type="button"
        className={`btn-view list asq-viewswitch__btn ${!isGrid ? "active" : ""}`}
        aria-pressed={!isGrid}
        onClick={() => setIsGrid(false)}
      >
        <ListIcon />
        <span className="asq-sr-only">{t("listView")}</span>
      </button>
    </div>
  );
}
